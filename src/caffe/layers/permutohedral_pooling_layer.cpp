...
void PermutohedralPoolingLayer<Dtype>::InitLattice(
  ...
  if (!operations_.empty() && (!operations_[0].max_
                               || do_repeated_init_)) {
  ...
  #pragma omp parallel
    {
      ...
      for (int n = 0; n < num_ ; ++n) {
        ...
        if (!in_lattice_blob_data) {
          ...
          // norm_type: PermutohedralPoolingParameter_NormType_INPUT: Non-symmetric
          ...
          switch (this->layer_param_.permutohedral_pooling_param().norm_type()) {
          case PermutohedralPoolingParameter_NormType_AFTER:
            op.max_->max_compute(gauss_filter, norm_there_data,
                              1, in_offset, out_offset, in_size, out_size,
                              norm_back_data);
          ...
          case PermutohedralPoolingParameter_NormType_SYMMETRIC:
            op.max_->max_compute(gauss_filter, norm_there_data,
                              1, in_offset, in_offset, in_size, in_size,
                              norm_there_data);

            std::fill(norm_back_data, norm_back_data + out_size, 1);
            op.max_->max_compute(gauss_filter, norm_back_data, 1,
...
#pragma omp for
    for (int n = 0; n < num_; ++n) {
      ...
      op.reverse_ = op.max_->max_compute(
          gauss_filter, scaled_there_data,
          channels_, in_offset, out_offset,
          in_size, out_size,
