...
class PermutohedralPoolingLayerTest : public MultiDeviceTest<TypeParam> {
 ...
 protected:
  static
  void Fill(const shared_ptr<Blob<Dtype> >& data,
...
struct TestSuite {
  static void TestSetup(const std::vector<Blob<Dtype>*>& bottom,
                        ...
                        int neighborhood_size, int num_output, int group) {
...
#define DEFINE_TYPED_TEST(test, type, neighborhood_size, num_output,    \
                          group, bottom)                                \
  TYPED_TEST(PermutohedralPoolingLayerTest,                                    \
             test ## _ ## type ## _ ## neighborhood_size ## _ ##        \
             num_output ## _ ## group ## _ ## bottom) {                 \
    typedef typename TypeParam::Dtype Dtype;                            \
    TestSuite<Dtype, permutohedral::type>::test(                        \
        this->bottom, this->top_,                                       \
        neighborhood_size, num_output, group);                          \
  }


DEFINE_TYPED_TEST(TestSetup, Permutohedral,
                  3, 1, 1, bottom_)
